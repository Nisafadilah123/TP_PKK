...
<div class="col-md-6">
    ...
    <div class="card card-primary">
      ...
      <form action="{{ url ('/data_aset_koperasi', $data_aset_koperasi->id) }}" method="POST">
      @method('PUT')
            ...
            <div class="form-group @error('id_desa') is-invalid @enderror">
                ...
                {{-- nama Nama Taman Bacaan --}}
                    <label>Nama Koperasi</label>
                        <input type="text" class="form-control @error('nama_koperasi') is-invalid @enderror" name="nama_koperasi" id="nama_koperasi" placeholder="Masukkan Nama Koperasi" value="{{ ucfirst(old('nama_koperasi', $data_aset_koperasi->nama_koperasi)) }}">
                            @error('nama_koperasi')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                        ...
                        <select class="form-control" id="jenis_koperasi" name="jenis_koperasi">
                            {{-- Pilih Koperasi --}}
                                @foreach($jenis_koperasi as $key => $val)
                                    @if($key==old('jenis_koperasi', $data_aset_koperasi->jenis_koperasi))
                ...
                {{-- nama Nama Pengelola --}}
                    <label>Status Hukum</label>
                        ...
                        @foreach($status_hukum as $key => $val)
                            @if($key==old('status_hukum', $data_aset_koperasi->status_hukum))
                                <option value="{{ $key }}" selected>{{ $val }}</option>
                            @else
                                <option value="{{ $key }}">{{ $val }}</option>
                            @endif
                ...
                {{-- Jumlah Buku --}}
                    <label>Jumlah Anggota Laki-laki</label>
                        <input type="number" min="0" class="form-control @error('jumlah_anggota_laki') is-invalid @enderror" name="jumlah_anggota_laki" id="jumlah_anggota_laki" placeholder="Masukkan Jumlah Anggota Koperasi Laki-laki" value="{{ ucfirst(old('jumlah_anggota_laki', $data_aset_koperasi->jumlah_anggota_laki)) }}">
                ...
                <div class="form-group">
                    ...
                    <label>Jumlah Anggota Perempuan</label>
                        <input type="number" min="0" class="form-control @error('jumlah_anggota_perempuan') is-invalid @enderror" name="jumlah_anggota_perempuan" id="jumlah_anggota_perempuan" placeholder="Masukkan Jumlah Anggota Koperasi Perempuan" value="{{ ucfirst(old('jumlah_anggota_perempuan', $data_aset_koperasi->jumlah_anggota_perempuan)) }}">
                ...
                <div class="form-group">
                    ...
                    <select style="cursor:pointer;" class="form-control" id="periode" name="periode">
                        <option value="{{ $data_aset_koperasi->periode }}" {{ $data_aset_koperasi->periode ? 'selected' : '' }}>{{ $data_aset_koperasi->periode }}</option>
                            <?php
                            $year = date('Y');
                            $min = $year ;
                                $max = $year + 20;
